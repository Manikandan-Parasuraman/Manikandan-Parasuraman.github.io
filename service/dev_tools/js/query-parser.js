document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("generate-btn").addEventListener("click", () => {
        ...
        try { try { const search = inputVal.includes('?') ? inputVal.split('?')[1] : inputVal; const params = new URLSearchParams(search); let out = ''; for (let [k,v] of params) out += k + ': ' + v + '\n'; output.value = out || 'No params found'; } catch(e) { output.value = 'Parse error'; } } catch(e) { 
            console.error(e);
