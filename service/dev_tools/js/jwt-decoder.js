document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("generate-btn").addEventListener("click", () => {
        ...
        try {
            try { const parts = inputVal.split('.'); const payload = JSON.parse(atob(parts[1])); output.value = JSON.stringify(payload, null, 4); } catch(e) { error.textContent = 'Invalid JWT'; error.style.display = 'block'; }
        } catch(e) { 
            console.error(e);
