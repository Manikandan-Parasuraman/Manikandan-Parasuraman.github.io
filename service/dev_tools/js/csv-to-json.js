document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("generate-btn").addEventListener("click", () => {
        ...
        try {
            const lines = inputVal.split('\n'); if(lines.length < 2) return; const headers = lines[0].split(','); const result = lines.slice(1).map(line => { const values = line.split(','); return headers.reduce((obj, header, i) => { obj[header.trim()] = values[i] ? values[i].trim() : ''; return obj; }, {}); }); output.value = JSON.stringify(result, null, 4);
        } catch(e) { 
            console.error(e);
