document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("generate-btn").addEventListener("click", () => {
        ...
        try {
            try { const arr = JSON.parse(inputVal); if(!Array.isArray(arr)) throw new Error(); const headers = Object.keys(arr[0]); const csv = [headers.join(','), ...arr.map(row => headers.map(h => row[h]).join(','))].join('\n'); output.value = csv; } catch(e) { error.textContent = 'Invalid JSON Array'; error.style.display = 'block'; }
        } catch(e) { 
            console.error(e);
